...
function computeSignals(articles: NewsArticle[]): Signal[] {
  return SIGNAL_DEFS.map((def) => {
    ...
    if (count === 0) {
      level = "gray";
    } else if (count >= 4 && avgTone < -2.5) {
      level = "red";
    } else if (count >= 2 || avgTone < -1.5) {
      level = "yellow";
    } else {
      level = "green";
    }
    ...
    return {
      ...
      avgTone,
      level,
      headlines: matching.slice(0, 3).map((a) => a.title),
    };
  });
}
...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [priceRes, invRes, energyRes, globalRes] = await Promise.allSettled([
        fetch("/api/energy-prices").then((r) => r.json()),
        fetch("/api/inventory").then((r) => r.json()),
        fetch("/api/energy-news").then((r) => r.json()),
        fetch("/api/global-news").then((r) => r.json()),
      ]);

      if (priceRes.status === "fulfilled") {
        setPrices(priceRes.value.data ?? null);
        setApiKeySet(priceRes.value.api_key_set ?? false);
      }
      if (invRes.status === "fulfilled") {
        setInventory(invRes.value.data ?? null);
      }
      if (energyRes.status === "fulfilled") {
        const arts: NewsArticle[] = energyRes.value.articles ?? [];
        setEnergyNews(arts);
        setSignals(computeSignals(arts));
      }
      if (globalRes.status === "fulfilled") {
        setGlobalNews(globalRes.value.articles ?? []);
      }
  ...
  useEffect(() => {
    fetchAll();
    const refreshId = setInterval(fetchAll, REFRESH_MS);
